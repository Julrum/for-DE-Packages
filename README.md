# for DE Packages

## Ubuntu Package List

### Library

1.  [build-essential](https://ubuntu.pkgs.org/)
2.  [gdebi](https://ubuntu.pkgs.org/)
3.  [libc6](https://ubuntu.pkgs.org/)
4.  [make](https://ubuntu.pkgs.org/)

### Theme

1.  [arc-theme](https://github.com/arc-design/arc-theme)
2.  [numix-theme](https://github.com/numixproject)
3.  [paper-theme](https://github.com/snwh/paper-gtk-theme)

### Tweak

1.  [gnome-tweak-tool](https://ubuntu.pkgs.org/)
2.  [plank](https://ubuntu.pkgs.org/)
3.  [unity-tweak-tool](https://ubuntu.pkgs.org/)

### Zsh

1.  [bat](https://github.com/sharkdp/bat)
2.  [fasd](https://github.com/clvv/fasd)
3.  [fd](https://github.com/sharkdp/fd)
4.  [fzf](https://github.com/junegunn/fzf)
5.  [install-oh-my-zsh](https://gist.github.com/hewerthomn/65bb351bf950470f6c9e6aba8c0c04f1)
6.  [k](https://github.com/supercrabtree/k)
7.  [marker](https://github.com/pindexis/marker)
8.  [pet](https://github.com/knqyf263/pet)
9.  [powerlevel10k](https://github.com/romkatv/powerlevel10k)
10.  [ripgrep](https://github.com/BurntSushi/ripgrep)
11.  [todotxt](https://github.com/todotxt/todo.txt-cli)
12.  [completions](https://github.com/zsh-users/zsh-completions)
13.  [history-substring-search](https://github.com/zsh-users/zsh-history-substring-search)

## etc

### fonts

1.  [D2Coding](https://github.com/naver/d2codingfont)

## VSCode extensions

### Theme

1.  [hipster](https://marketplace.visualstudio.com/items?itemName=ModoNoob.vscode-hipster-theme)